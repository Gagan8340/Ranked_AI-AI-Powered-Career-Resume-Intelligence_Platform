    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            for sug in suggestions:
                ...
                if s_type == "skill":
                    # NEVER insert missing skills automatically without evidence. We skip them.
                    continue
                    ...
                    cursor.execute("SELECT 1 FROM user_projects WHERE user_id = %s AND project_name = %s", (user_id, s_val[:255]))
                    if not cursor.fetchone():
                        cursor.execute("INSERT INTO user_projects (user_id, project_name, description) VALUES (%s, %s, %s)", (user_id, s_val[:255], "Added via JD Optimization - Please review"))
                        
            # 5. Update builder_profile
            cursor.execute(\"\"\"
                INSERT INTO builder_profiles (user_id, professional_summary, skills, experience)
                VALUES (%s, %s, %s, %s)
                ON DUPLICATE KEY UPDATE
                professional_summary = VALUES(professional_summary),
                skills = VALUES(skills),
                experience = VALUES(experience)
            \"\"\", (user_id, current_summary.strip(), json.dumps(current_skills), json.dumps(current_experience)))
            
            # 6. Rebuild the resume text to calculate the new ATS Score
            ...
            # Update optimized resume in builder_profiles
            cursor.execute("UPDATE builder_profiles SET optimized_resume_text = %s WHERE user_id = %s", (new_resume_text, user_id))
            ...
            # Get full builder state for version_data
            cursor.execute("SELECT * FROM builder_profiles WHERE user_id = %s", (user_id,))
            ...
            \"\"\", (user_id, resume_id, jd_id, json.dumps(full_profile, default=str), new_resume_text, improvement_score, next_version))
            
            # 9. Invalidate ATS Cache
